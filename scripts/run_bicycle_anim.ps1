# Render Mip-NeRF 360 bicycle reconstruction frames to 30k force-full for baseline
# and agentic (+per-block actions), then rebuild the interactive scrubber assets.
...
Write-Output "=== STAGE 1: render bicycle baseline frames (to 30k) ==="
...
Write-Output "=== STAGE 2: render bicycle agentic frames (to 30k, force-full, +actions) ==="
& conda run --no-capture-output -n env_pytorch_3dgs python -u $rend `
...
Write-Output "=== STAGE 3: rebuild scrubber assets (incl. bicycle) ==="
